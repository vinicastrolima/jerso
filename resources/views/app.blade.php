...
<head>
  ...
  <title>AlaPoker • Gerenciador de Mesas & Bankroll</title>
  ...
  <style>
    body {
      ...
      background-image: 
        ...
        radial-gradient(at 50% 100%, rgba(30, 41, 59, 0.5) 0px, transparent 60%);
      color: #f1f5f9;
      min-height: 100vh;
...
<body class="antialiased flex flex-col justify-between">
  ...
  <!-- Top Navigation Bar -->
  <header class="sticky top-0 z-30 border-b border-white/[0.07] bg-poker-950/80 backdrop-blur-xl">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
      
      <!-- Brand & Table Indicator -->
      <div class="flex items-center gap-3">
        <a href="/" onclick="switchTab('table', event)" class="flex items-center gap-2.5 group">
          <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center shadow-lg shadow-emerald-500/20 text-white font-bold group-hover:scale-105 transition-transform">
            ♠️
          </div>
          <div>
            <div class="flex items-center gap-2">
              <span class="font-extrabold text-base tracking-tight text-white">AlaPoker</span>
              <span class="text-[10px] uppercase font-bold tracking-widest px-1.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">PRO</span>
            </div>
            <p class="text-xs text-slate-400 hidden sm:block">Gerenciador de Mesas & Bankroll</p>
          </div>
        </a>

        <!-- Mesa Atual Pill (clicável para trocar de mesa) -->
        <div id="topTableIndicator" onclick="openTableSwitcherModal()" class="hidden items-center gap-1.5 pl-2 sm:pl-3 border-l border-white/10 cursor-pointer hover:bg-white/5 py-1 px-2 rounded-xl transition-all" title="Clique para trocar de mesa">
          <span class="relative flex h-2 w-2">
            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
            <span class="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
          </span>
          <span id="topTableName" class="text-xs font-semibold text-slate-200 truncate max-w-[80px] sm:max-w-[180px]">Mesa</span>
          <span id="topTableCode" class="font-mono text-[10px] px-1.5 py-0.5 rounded bg-white/5 border border-white/10 text-slate-400"></span>
          <i data-lucide="chevron-down" class="w-3 h-3 text-slate-400"></i>
        </div>
      </div>

      <!-- Desktop Nav Tabs -->
      <nav class="hidden md:flex items-center gap-1 bg-white/[0.04] p-1 rounded-xl border border-white/[0.08]">
        <button onclick="switchTab('table')" id="nav-tab-table" class="nav-tab-btn px-4 py-1.5 rounded-lg text-sm font-medium transition-all flex items-center gap-2 text-white bg-emerald-500/20 border border-emerald-500/30">
          <i data-lucide="layout-grid" class="w-4 h-4"></i> Mesa
        </button>
        <button onclick="switchTab('ranking')" id="nav-tab-ranking" class="nav-tab-btn px-4 py-1.5 rounded-lg text-sm font-medium transition-all flex items-center gap-2 text-slate-400 hover:text-white hover:bg-white/5">
          <i data-lucide="trophy" class="w-4 h-4"></i> Hall da Fama
        </button>
        <button onclick="switchTab('players')" id="nav-tab-players" class="nav-tab-btn px-4 py-1.5 rounded-lg text-sm font-medium transition-all flex items-center gap-2 text-slate-400 hover:text-white hover:bg-white/5">
          <i data-lucide="users" class="w-4 h-4"></i> Jogadores
        </button>
        <button onclick="switchTab('admin')" id="nav-tab-admin" class="nav-tab-btn px-4 py-1.5 rounded-lg text-sm font-medium transition-all flex items-center gap-2 text-slate-400 hover:text-white hover:bg-white/5">
    ...
    <section id="section-table" class="space-y-6">
      ...
      <div id="noTableState" class="hidden">
        <div class="glass-card rounded-3xl p-8 sm:p-12 text-center max-w-lg mx-auto my-8 border border-white/[0.08] shadow-2xl relative overflow-hidden">
          ...
          <h2 class="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">Nenhuma mesa aberta</h2>
          <p class="text-slate-400 text-sm mt-2 mb-6">Inicie uma nova mesa de poker ou entre com o código de uma mesa existente para acompanhar ao vivo.</p>

          <div class="flex flex-col sm:flex-row gap-3 justify-center">
            <button onclick="openNewTableModal()" class="px-5 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm flex items-center justify-center gap-2 shadow-lg shadow-emerald-600/25 transition-all active:scale-95">
              <i data-lucide="plus-circle" class="w-4 h-4"></i> Criar Nova Mesa
            </button>
            <button onclick="openEnterCodeModal()" class="px-5 py-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-slate-200 font-semibold text-sm flex items-center justify-center gap-2 transition-all active:scale-95">
              <i data-lucide="key-round" class="w-4 h-4"></i> Entrar por Código
            </button>
          </div>
      ...
      <div id="activeTableState" class="space-y-6">
        ...
        <div class="glass-card rounded-2xl p-4 sm:p-6 border border-white/[0.08]">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div>
              <div class="flex items-center gap-2.5 flex-wrap">
                <span id="tableLiveStatusBadge" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-extrabold tracking-wide uppercase bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                  <span class="w-2 h-2 rounded-full bg-emerald-400 animate-ping"></span> AO VIVO
                </span>
            ...
            <div class="flex items-center gap-2 flex-wrap w-full sm:w-auto">
              ...
              <button id="btnCloseTableBtn" onclick="openCloseTableModal()" class="hidden w-full sm:w-auto px-3.5 py-2 rounded-xl bg-red-500/15 hover:bg-red-500/25 border border-red-500/30 text-red-400 text-xs font-bold items-center justify-center gap-1.5 transition-all active:scale-95">
                <i data-lucide="lock" class="w-3.5 h-3.5"></i> Encerrar Mesa
              </button>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-6">
            ...
            <div class="glass-card rounded-xl p-3.5 sm:p-4 border border-white/[0.06] relative overflow-hidden">
              ...
              <div id="statTotalMoney" class="text-lg sm:text-2xl font-extrabold font-mono text-emerald-400 mt-1 tabular-nums">R$ 0,00</div>
            ...
            <div class="glass-card rounded-xl p-3.5 sm:p-4 border border-white/[0.06]">
              ...
              <div id="statTopWinnerName" class="text-sm sm:text-base font-bold text-amber-300 mt-1 truncate">—</div>
          ...
          <div id="tableClosedBanner" class="hidden mt-5 p-4 rounded-2xl bg-gradient-to-r from-emerald-500/15 via-emerald-600/10 to-transparent border border-emerald-500/30 flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xl">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-sm font-bold text-white">Esta mesa foi encerrada e salva!</p>
                <p class="text-xs text-slate-300">Pronto para a próxima rodada com os amigos?</p>
              </div>
            </div>
            <div class="flex items-center gap-2 w-full sm:w-auto">
              <button onclick="openNewTableModal()" class="flex-1 sm:flex-initial px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-xs flex items-center justify-center gap-1.5 shadow-lg shadow-emerald-600/25 active:scale-95 transition-all">
                <i data-lucide="plus-circle" class="w-4 h-4"></i> Criar Nova Mesa
              </button>
              <button onclick="openTableSwitcherModal()" class="px-3 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 font-bold text-xs flex items-center justify-center gap-1.5 active:scale-95 transition-all">
                <i data-lucide="layers" class="w-4 h-4"></i> Outras Mesas
              </button>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div class="glass-card rounded-2xl p-4 sm:p-6 border border-white/[0.08]">
            <div class="flex items-center justify-between mb-4">
              <div class="flex items-center gap-2">
                <i data-lucide="medal" class="w-5 h-5 text-amber-400"></i>
                <h3 class="text-base font-bold text-white">Classificação da Mesa</h3>
              </div>
              <span class="text-xs text-slate-400">Lucro / Prejuízo</span>
            </div>
          ...
          <div class="glass-card rounded-2xl p-4 sm:p-6 border border-white/[0.08]">
            <div class="flex items-center justify-between mb-4">
              <div class="flex items-center gap-2">
                ...
                <h3 class="text-base font-bold text-white">Acertos (Quem Paga Quem)</h3>
              </div>
              <button onclick="copyWhatsAppSummary()" class="px-2.5 py-1 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 text-emerald-400 text-xs font-semibold flex items-center gap-1 transition-all">
                <i data-lucide="copy" class="w-3.5 h-3.5"></i> Copiar Resumo
              </button>
            </div>
      ...
      <div class="glass-card rounded-2xl p-5 sm:p-8 border border-white/[0.08]">
        ...
        <div class="mt-8 overflow-x-auto">
          <table class="w-full text-left text-sm border-collapse">
            <thead>
              <tr class="border-b border-white/10 text-xs uppercase font-bold text-slate-400 tracking-wider">
                <th class="py-3 px-3">Posição</th>
                <th class="py-3 px-3">Jogador</th>
                <th class="py-3 px-3 text-right">Mesas</th>
                <th class="py-3 px-3 text-right">Vitórias</th>
                <th class="py-3 px-3 text-right">Taxa Vit.</th>
                <th class="py-3 px-3 text-right">Maior Forra</th>
                <th class="py-3 px-3 text-right">Lucro Geral</th>
              </tr>
            </thead>
            <tbody id="hallOfFameTableBody" class="divide-y divide-white/5"></tbody>
          </table>
        </div>
      </div>
    </section>
      ...
      <div id="adminUnlockedState" class="hidden space-y-6">
        ...
        <div class="glass-card rounded-2xl p-5 sm:p-8 border border-white/[0.08]">
          <div class="flex items-center justify-between mb-6">
            <div>
              <h1 class="text-xl sm:text-2xl font-black text-white">Painel da Banca & Histórico</h1>
              <p class="text-xs text-slate-400">Visão macro de todas as mesas e movimentação financeira</p>
            </div>
            <button onclick="lockAdmin()" class="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-slate-400 text-xs font-semibold">
              Bloquear Admin
            </button>
          </div>
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="p-4 rounded-xl bg-white/[0.02] border border-white/5">
              <span class="text-xs text-slate-400">Total de Mesas</span>
              <div id="adminTotalTables" class="text-xl sm:text-2xl font-mono font-bold text-white mt-1 tabular-nums">0</div>
            </div>
            <div class="p-4 rounded-xl bg-white/[0.02] border border-white/5">
              <span class="text-xs text-slate-400">Média por Pote</span>
  ...
  <div id="modalTableSwitcher" class="fixed inset-0 z-50 hidden bg-black/75 backdrop-blur-sm items-center justify-center p-4 overflow-y-auto">
    <div class="glass-card rounded-3xl p-6 sm:p-8 max-w-md w-full border border-white/10 shadow-2xl relative my-6">
      ...
      <div id="tableSwitcherList" class="space-y-2 max-h-64 overflow-y-auto pr-1 my-3"></div>

      <div class="pt-3 border-t border-white/10 flex flex-col sm:flex-row gap-2">
        <button onclick="closeModal('modalTableSwitcher'); openNewTableModal();" class="flex-1 py-2.5 px-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-xs flex items-center justify-center gap-1.5 shadow-lg shadow-emerald-600/20">
          <i data-lucide="plus-circle" class="w-4 h-4"></i> + Nova Mesa
        </button>
        <button onclick="closeModal('modalTableSwitcher'); openEnterCodeModal();" class="py-2.5 px-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 font-bold text-xs flex items-center justify-center gap-1.5">
          <i data-lucide="key-round" class="w-4 h-4"></i> Entrar por Código
        </button>
      </div>
    </div>
  ...
  <!-- Toast Notification -->
  <div id="toast" class="fixed bottom-20 md:bottom-6 right-4 left-4 sm:left-auto sm:right-6 max-w-md z-50 pointer-events-none opacity-0 translate-y-4 transition-all duration-300">
    <div id="toastContent" class="px-4 py-3 rounded-2xl bg-poker-850 border border-white/15 text-white text-xs font-semibold shadow-2xl flex items-center gap-2.5">
      <i id="toastIcon" data-lucide="info" class="w-4 h-4 text-emerald-400 shrink-0"></i>
  ...
  <script>
    ...
    function switchTab(tabName, event) {
      ...
      if (tabName === 'ranking') loadHallOfFame();
      if (tabName === 'players') loadPlayersCatalog();
    ...
    async function apiFetch(url, options = {}) {
      ...
      if (state.managerPin) {
        headers['X-Table-Pin'] = state.managerPin;
      }
    ...
    function renderActiveTable(data) {
      ...
      const topWinnerEl = document.getElementById('statTopWinnerName');
      const topWinnerProfEl = document.getElementById('statTopWinnerProfit');
      if (data.summary.top_winner) {
        topWinnerEl.textContent = data.summary.top_winner.name;
        topWinnerProfEl.textContent = `+ ${fmtMoney(data.summary.top_winner.profit)}`;
      } else {
        topWinnerEl.textContent = 'Aguardando saídas';
      ...
      const btnDesktopAddPlayer = document.getElementById('btnDesktopAddPlayer');
      ...
      // Toggle do Banner de Mesa Encerrada
      const closedBanner = document.getElementById('tableClosedBanner');
      if (closedBanner) {
        if (isClosed) {
          closedBanner.classList.remove('hidden');
          closedBanner.classList.add('flex');
        } else {
          closedBanner.classList.add('hidden');
          closedBanner.classList.remove('flex');
        }
      }
      ...
      try {
        ...
        if (res.tables && res.tables.length > 0) {
          container.innerHTML = res.tables.map(t => {
            const isCurrent = t.code === state.activeCode;
            const isTableClosed = t.status === 'closed';
            return `
              <div onclick="switchTable('${t.code}'); closeModal('modalTableSwitcher');" class="p-3 rounded-xl bg-white/[0.03] hover:bg-white/[0.08] border border-white/5 flex items-center justify-between cursor-pointer transition-all ${isCurrent ? 'border-emerald-500/40 bg-emerald-500/10' : ''}">
                <div>
                  <div class="flex items-center gap-2">
                    <span class="font-bold text-xs text-white">${escapeHtml(t.name)}</span>
                    ${isCurrent ? '<span class="text-[9px] uppercase font-black px-1.5 py-0.2 rounded bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">Atual</span>' : ''}
                  </div>
                  <span class="font-mono text-[10px] text-slate-400 block mt-0.5">${t.code} • ${t.players_count} jogadores</span>
                </div>
                <span class="text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${isTableClosed ? 'bg-slate-500/20 text-slate-400 border border-slate-500/30' : 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'}">
                  ${isTableClosed ? 'Encerrada' : 'Ao Vivo'}
                </span>
              </div>
            `;
          }).join('');
        } else {
          container.innerHTML = '<div class="text-xs text-slate-400 py-4 text-center">Nenhuma outra mesa cadastrada.</div>';
        }
        openModal('modalTableSwitcher');
      } catch {}
    }
      ...
      container.innerHTML = ranking.map((r, idx) => {
        ...
        return `
          <div class="flex items-center justify-between p-2.5 rounded-xl bg-white/[0.02] border border-white/5">
            <div class="flex items-center gap-2.5">
              <span class="w-6 text-center font-bold text-xs text-slate-400">${medal}</span>
              <span class="font-semibold text-sm text-white">${escapeHtml(r.name)}</span>
            </div>
            <div>${valBadge}</div>
          </div>
        `;
      ...
      container.innerHTML = settlements.map(s => `
        <div class="p-3 rounded-xl bg-white/[0.02] border border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div class="flex items-center gap-2 text-xs">
            <b class="text-red-400">${escapeHtml(s.from)}</b>
      ...
      try {
        ...
        if (res.tables && res.tables.length > 0) {
          container.innerHTML = res.tables.slice(0, 5).map(t => `
            <div onclick="switchTable('${t.code}')" class="p-2.5 rounded-xl bg-white/[0.03] hover:bg-white/[0.08] border border-white/5 flex items-center justify-between cursor-pointer transition-all">
              <div>
                <span class="font-bold text-xs text-white">${escapeHtml(t.name)}</span>
                <span class="font-mono text-[10px] text-slate-400 block">${t.code} • ${t.players_count} jogadores</span>
              </div>
              <span class="text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${t.status === 'active' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-500/20 text-slate-400'}">
                ${t.status === 'active' ? 'Ao Vivo' : 'Encerrada'}
              </span>
            </div>
          `).join('');
        } else {
    ...
    function switchTable(code) {
      ...
      state.managerPin = sessionStorage.getItem(`alapoker_pin_${code}`) || null;
      loadTableData();
      switchTab('table');
    }
    ...
    async function submitPinUnlock(e) {
      e.preventDefault();
      ...
      try {
        ...
        if (res.success) {
          ...
          sessionStorage.setItem(`alapoker_pin_${state.activeCode}`, pin);
          closeModal('modalPin');
          document.getElementById('tablePinInput').value = '';
    ...
    function openAddBuyinModal(tablePlayerId, playerName, totalBuyin) {
      ...
      document.getElementById('addBuyinPlayerName').textContent = `${playerName} (Já colocou ${fmtMoney(totalBuyin)})`;
      document.getElementById('addBuyinCustomAmount').value = state.selectedBuyinPreset || 100;
      openModal('modalAddBuyin');
    }

    function setBuyinPreset(val) {
      state.selectedBuyinPreset = val;
      document.getElementById('addBuyinCustomAmount').value = val;
    }

    async function submitAddBuyin(e) {
      e.preventDefault();
      ...
      try {
        ...
        closeModal('modalAddBuyin');
        showToast(res.message);
        loadTableData();
      } catch {}
    }

    let currentCashoutTablePlayerId = null;
    let currentCashoutTotalBuyin = 0;
    function openCashoutModal(tablePlayerId, playerName, totalBuyin) {
      ...
      document.getElementById('cashoutPreviewValue').textContent = '—';
    ...
    function updateCashoutPreview() {
      ...
      if (diff > 0) {
        ...
        preview.className = 'text-sm font-bold font-mono text-red-400 mt-0.5';
        preview.textContent = `PREJUÍZO DE - ${fmtMoney(Math.abs(diff))}`;
      } else {
        preview.className = 'text-sm font-bold font-mono text-amber-400 mt-0.5';
        preview.textContent = 'ZERO A ZERO';
      }
    }

    async function submitCashout(e) {
      e.preventDefault();
    ...
    function openEditPlayerModal(tablePlayerId) {
      ...
      const p = state.tableData?.players.find(x => x.id === tablePlayerId);
      if (!p) return;

      document.getElementById('editPlayerModalTitle').textContent = `Editar ${p.name}`;
      document.getElementById('editPlayerModalSubtitle').textContent = `Total de entradas: ${fmtMoney(p.total_buyin)}`;

      const buyinsContainer = document.getElementById('editBuyinsListContainer');
      buyinsContainer.innerHTML = p.buyins.map((b, idx) => `
        <div class="p-2.5 rounded-xl bg-white/[0.03] border border-white/5 flex items-center justify-between text-xs">
          <div>
            <span class="font-bold font-mono text-white">${fmtMoney(b.amount)}</span>
      ...
      const cashedOutSection = document.getElementById('editCashedOutSection');
      if (p.status === 'cashed_out') {
        cashedOutSection.classList.remove('hidden');
        document.getElementById('editFinalAmountInput').value = p.final_amount ?? '';
      } else {
        cashedOutSection.classList.add('hidden');
      }
    ...
    async function submitNewBuyinFromEditModal() {
      ...
      try {
        ...
        document.getElementById('editModalNewBuyin').value = '';
        showToast(res.message);
        loadTableData();
    ...
    async function submitReopenPlayer() {
      await quickReopen(currentEditingPlayerId);
      ...
      if (state.playersCatalog.length === 0) {
        ...
        container.innerHTML = state.playersCatalog.map(p => `
          <label class="flex items-center justify-between p-2 rounded-lg hover:bg-white/5 cursor-pointer text-xs">
            ...
            <span class="text-[11px] font-mono text-emerald-400">R$ 100</span>
          </label>
        `).join('');
      }
    ...
    function selectAllNewTablePlayers() {
      const boxes = document.querySelectorAll('input[name="selected_players"]');
    ...
    function openCloseTableModal() {
      ...
      if (activePlayers.length === 0) {
        ...
        container.innerHTML = `
          <div class="text-xs text-amber-400 font-semibold mb-2">Jogadores ainda com fichas na mesa:</div>
          ${activePlayers.map(p => `
            <div class="p-2.5 rounded-xl bg-white/[0.03] border border-white/5 flex items-center justify-between text-xs">
              <div>
                <span class="font-bold text-white">${escapeHtml(p.name)}</span>
                <span class="text-slate-400 text-[11px] block">Entradas: ${fmtMoney(p.total_buyin)}</span>
              </div>
              <input type="number" step="0.01" data-player-id="${p.id}" placeholder="Final (R$)" class="close-final-input w-28 py-1.5 px-2 rounded-lg bg-black/40 border border-white/15 text-white font-mono text-right text-xs outline-none" />
            </div>
          `).join('')}
        `;
      }
    ...
    async function submitCloseTable() {
      const inputs = document.querySelectorAll('.close-final-input');
      const finalAmounts = {};
      inputs.forEach(input => {
        const pId = input.dataset.playerId;
        const val = parseFloat(input.value);
        finalAmounts[pId] = isNaN(val) ? 0 : val;
      });
      ...
      podium.innerHTML = top3.map((p, idx) => {
        ...
        return `
          <div class="glass-card rounded-2xl p-5 border ${borderColors[idx]} text-center relative overflow-hidden">
            ...
            <div class="w-14 h-14 mx-auto rounded-2xl flex items-center justify-center text-white font-extrabold text-base my-3 shadow-lg" style="background-color: ${p.avatar_color || '#10b981'}">
            ...
            <div class="mt-4 pt-3 border-t border-white/5 grid grid-cols-2 gap-2 text-xs">
              <div>
                ...
                <span class="font-mono font-bold ${profitClass}">${p.net_profit >= 0 ? '+' : ''}${fmtMoney(p.net_profit)}</span>
              </div>
              <div>
                <span class="text-[10px] text-slate-400 block uppercase">Taxa Vitória</span>
                <span class="font-mono font-bold text-white">${p.win_rate}%</span>
              </div>
            </div>
          </div>
        `;
      ...
      tableBody.innerHTML = list.map((p, idx) => {
        ...
        return `
          <tr class="hover:bg-white/[0.02] text-xs">
            <td class="py-3 px-3 font-mono font-bold text-slate-400">#${idx + 1}</td>
            <td class="py-3 px-3">
              <div class="flex items-center gap-2">
                <span class="w-2 h-2 rounded-full" style="background-color: ${p.avatar_color || '#10b981'}"></span>
                <span class="font-bold text-white">${escapeHtml(p.name)}</span>
                ${p.nickname ? `<span class="text-slate-500 text-[11px]">@${escapeHtml(p.nickname)}</span>` : ''}
              </div>
            </td>
            <td class="py-3 px-3 text-right font-mono text-slate-300">${p.sessions_count}</td>
            <td class="py-3 px-3 text-right font-mono text-slate-300">${p.wins_count}</td>
            <td class="py-3 px-3 text-right font-mono text-slate-300">${p.win_rate}%</td>
            <td class="py-3 px-3 text-right font-mono text-emerald-400">${fmtMoney(p.best_win)}</td>
            <td class="py-3 px-3 text-right font-mono ${profitClass}">${p.net_profit >= 0 ? '+' : ''}${fmtMoney(p.net_profit)}</td>
          </tr>
        `;
      }).join('');
    }
    ...
    function renderPlayersCatalog(players) {
      ...
      grid.innerHTML = players.map(p => `
        <div class="glass-card rounded-2xl p-4 border border-white/[0.06] flex items-center justify-between gap-3">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-xs text-white shrink-0 shadow-md" style="background-color: ${p.avatar_color || '#10b981'}">
              ${p.name.slice(0, 2).toUpperCase()}
            </div>
            <div>
              <span class="font-bold text-sm text-white block truncate max-w-[140px]">${escapeHtml(p.name)}</span>
              ${p.nickname ? `<span class="text-xs text-slate-400 block">@${escapeHtml(p.nickname)}</span>` : ''}
    ...
    function openNewPlayerModal() {
      document.getElementById('playerModalTitle').textContent = 'Cadastrar Amigo';
      document.getElementById('editPlayerDirectoryId').value = '';
      document.getElementById('directoryPlayerName').value = '';
      document.getElementById('directoryPlayerNickname').value = '';
      document.getElementById('directoryPlayerPix').value = '';
      document.getElementById('directoryPlayerColor').value = '#10b981';
      openModal('modalNewPlayer');
    }
    ...
    function editDirectoryPlayer(id) {
      ...
      document.getElementById('directoryPlayerNickname').value = p.nickname || '';
      document.getElementById('directoryPlayerPix').value = p.pix_key || '';
      document.getElementById('directoryPlayerColor').value = p.avatar_color || '#10b981';
      openModal('modalNewPlayer');
    }

    async function submitSavePlayer(e) {
      e.preventDefault();
      const id = document.getElementById('editPlayerDirectoryId').value;
      const name = document.getElementById('directoryPlayerName').value.trim();
    ...
    function lockAdmin() {
      ...
      sessionStorage.removeItem('alapoker_admin_unlocked');
    ...
    async function loadAdminOverview() {
      if (sessionStorage.getItem('alapoker_admin_unlocked') === 'true') {
        state.adminUnlocked = true;
        document.getElementById('adminLockedState').classList.add('hidden');
        document.getElementById('adminUnlockedState').classList.remove('hidden');
      }
    ...
    async function viewHistoryDetail(id) {
      try {
        ...
        let settlementsHtml = (res.settlements || []).map(s => `
          <div class="p-2 rounded-lg bg-white/[0.02] text-xs flex justify-between">
            <span><b>${escapeHtml(s.from)}</b> paga <b>${escapeHtml(s.to)}</b></span>
            <span class="font-mono font-bold text-white">${fmtMoney(s.value)}</span>
          </div>
        `).join('');

        container.innerHTML = `
          <div class="space-y-2">
            <span class="text-xs uppercase font-bold text-slate-400">Classificação Final</span>
            ${rankingHtml || '<div class="text-xs text-slate-500">Sem ranking registrado</div>'}
          </div>
          <div class="space-y-2 pt-3 border-t border-white/10">
            <span class="text-xs uppercase font-bold text-slate-400">Acertos Registrados</span>
